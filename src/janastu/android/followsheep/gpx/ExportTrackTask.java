...
import janastu.android.followsheep.util.FileSystemUtils;
 ...
 */
	protected String buildZipFilename(Cursor c) {
		StringBuffer filenameBase = new StringBuffer();
		final long startDate = c.getLong(c.getColumnIndex(Schema.COL_START_DATE));
		if (filenameBase.length() > 0)
			filenameBase.append('_');
	...
	filenameBase.append(".zip");
	return filenameBase.toString();
	}
/**
 * Write text file in SD Card containing information about users and Device ID 
 */
	private void newConfigFile(){
		try {
			String deviceId = DataHelper.getDeviceId(context);
			String data = "User:"+" "+PreferenceManager.getDefaultSharedPreferences(context).getString(
					OSMTracker.Preferences.KEY_USER_NAME,
					OSMTracker.Preferences.VAL_USER_NAME)+"\n"+"Device ID:"+" "+deviceId;
			File trackDir = DataHelper.getTrackDirectory(trackId);
			if (trackDir.exists() && trackDir.canWrite()) {
			File newFile = new File(trackDir +File.separator+"config.txt");
			newFile.createNewFile();
			 Writer	writer = new BufferedWriter(new FileWriter(newFile));
		      writer.write(data);
		      writer.close();
			Log.d(TAG, "New config file: " + newFile.getAbsolutePath());
			}
		} catch (IOException ioe) {
			Log.e(TAG, "Could not create config file", ioe);
			throw new IllegalStateException("Could not create config file", ioe);
		}
	}
